...
SCAN_SUFFIXES = {
    ...
    ".md", ".sql", ".sh", ".env", ".ini", ".cfg", ".toml", ".txt", ".ipynb",
    ".tf", ".conf", ".properties", ".xml", ".html", ".css",
}
...
LOCAL_HOSTS = {
    ...
    "postgres", "postgresql", "mongo", "mongodb", "db", "database", "backend",
}
...
PATTERNS: list[tuple[str, re.Pattern[str], str]] = [
    ...
    ("Slack token", re.compile(r"\bxox[abprs]-[0-9A-Za-z-]{10,}\b"), "Revoke it in Slack."),
    ("Google API key", re.compile(r"\bAIza[0-9A-Za-z_-]{35}\b"),
     "Revoke it in the Google Cloud console."),
    ("OpenAI-style API key", re.compile(r"\bsk-[A-Za-z0-9]{32,}\b"), "Revoke the key."),
    ("Anthropic API key", re.compile(r"\bsk-ant-[A-Za-z0-9_-]{20,}\b"), "Revoke the key."),
    ("JSON Web Token", re.compile(r"\beyJ[A-Za-z0-9_-]{10,}\.eyJ[A-Za-z0-9_-]{10,}\."),
     "Tokens are short lived, but never commit one."),
    ("Hardcoded secret assignment",
     re.compile(r"\b(?:secret_key|api_key|apikey|access_token|auth_token|client_secret|"
                r"private_key|password|passwd)\s*[:=]\s*['\"]([^'\"\s]{12,})['\"]", re.I),
...
def should_skip(relative: str) -> bool:
    ...
    return any(relative.startswith(prefix) or relative == prefix for prefix in SKIP_PATHS)


def check_database_urls(line: str, relative: str, line_number: int, report: Report) -> None:
...
def main() -> int:
    ...
    for file_path in tracked_files():
        ...
        if should_skip(relative):
            continue
        if Path(relative).suffix.lower() not in SCAN_SUFFIXES and name != ".env.example":
            continue
        try:
