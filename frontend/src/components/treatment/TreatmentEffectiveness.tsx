...
import type { TreatmentEffectiveness } from '@/types';
...
export default function TreatmentEffectiveness() {
  ...
  useEffect(() => {
    async function loadTreatmentData() {
      try {
        const result = await apiFetch<TreatmentEffectiveness[]>(
          '/treatment',
        );
        setData(result);
      } catch {
        ...
        <>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <div className="rounded-xl border border-[var(--border)] bg-[var(--surface)] p-5">
              <p className="text-sm text-slate-500">Treatments tracked</p>
            ...
            <div className="rounded-xl border border-[var(--border)] bg-[var(--surface)] p-5">
              ...
              <p className="mt-2 text-3xl font-bold">
                {data
                  .reduce((total, item) => total + item.patients_treated, 0)
                  .toLocaleString()}
              </p>
            </div>

            <div className="rounded-xl border border-[var(--border)] bg-[var(--surface)] p-5">
              <p className="text-sm text-slate-500">Average readmission rate</p>
              <p className="mt-2 text-3xl font-bold">
                {(
                  (data.reduce(
                    (total, item) => total + item.readmission_rate,
                    0,
                  ) /
                    data.length) *
                  100
              ...
              <tbody>
                {data.map((item) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-4">
                      {item.average_recovery_score.toFixed(2)}
                    </td>
                    <td className="px-5 py-4">
