...
export function LoginForm() {
  ...
  return (
    <div className="w-full space-y-6">
      {loginSuccess && (
        <Alert variant="success" title="Authentication Successful">
          ...
          <div className="mt-2">
            <Link
              href="/dashboard"
              className="inline-flex items-center text-xs font-semibold text-sage-800 dark:text-sage-300 underline hover:no-underline"
            >
      ...
      <form onSubmit={handleSubmit} noValidate className="space-y-4">
        ...
        <div className="flex items-center justify-between pt-1">
          ...
          <Link
            ...
            className="text-xs font-semibold text-brand-500 hover:text-brand-600 dark:text-brand-400 dark:hover:text-brand-300 focus-visible:outline-none focus-visible:underline"
          >
            Forgot password?
          </Link>
        </div>
      ...
      <div className="rounded-xl border border-dashed border-warm-border bg-warm-neutral/30 p-4 dark:border-warm-border dark:bg-warm-neutral/10">
        <div className="flex items-center gap-2 mb-2.5">
          <PulseIcon className="h-3.5 w-3.5 text-brand-500 dark:text-brand-400" />
          <span className="text-xs font-semibold text-warm-text dark:text-warm-text">
        ...
        <div className="grid grid-cols-2 gap-2">
          {DEMO_ACCOUNTS.map((acc) => (
            <button
              ...
              className="flex flex-col items-start rounded-lg border border-warm-border bg-white p-2 text-left hover:border-brand-500 hover:bg-brand-50/40 dark:border-warm-border dark:bg-warm-card dark:hover:border-brand-400 text-xs transition-colors"
            >
              <span className="font-semibold text-warm-text dark:text-warm-text">
                {acc.label}
              </span>
              <span className="text-[10px] text-warm-text-muted truncate w-full">
                {acc.email}
              </span>
            </button>
          ))}
      ...
      {/* Switch to Register */}
      <div className="text-center text-xs text-warm-text-muted dark:text-warm-text-muted">
