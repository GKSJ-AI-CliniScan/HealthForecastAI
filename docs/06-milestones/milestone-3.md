...
## Why this is measured on the real dataset

Every number in this report and in both artefacts is computed from
`ml/data/raw/diabetic_data.csv` - the Diabetes 130-US Hospitals records, 101,766
encounters from 130 US hospitals between 1999 and 2008. Nothing is a worked
example, a synthetic table or a hand-written fixture. The pipeline that produces
them is `ml/src/evaluation/treatment_report.py`, and it can be re-run from a
clean clone to get the same numbers.

Two things make that checkable rather than a claim:

- **The analysis population is the training population.** `treatment_report.py`
  calls the same `basic_clean` and `build_features` the training entrypoint
  calls, so the 69,987 patients the treatment tables describe are the patients
  the model was fitted on - first encounter per patient, death and hospice
  discharges removed. A separate cleaning path would have produced numbers that
  quietly described a different group of people.
- **The model evaluation reproduces `metrics.json` exactly.** ROC-AUC 0.6518,
  recall 0.5099, precision 0.1454, to every digit already recorded in Milestone 2.
  It matches because the test split is rebuilt from the same config values and
  the same `random_state`, not re-randomised. If the split had drifted, these
  numbers would have drifted with it, so the match is evidence the analysis is
  running on the held-out rows it claims to.

The dataset's own fixtures are used only in the unit tests, and only for edge
cases the real data does not contain - an all-null row, a group of one, a drug
column with a single value. Those exist to prove the code handles cases it will
eventually meet, not to produce any reported figure.

### What the real data cannot support, stated plainly

Using real records does not make every question answerable. Three limits are
properties of this dataset and no amount of modelling removes them:

- **No HbA1c improvement can be measured.** There is one `A1Cresult` per
  encounter and no follow-up value. Any "HbA1c improved after treatment" claim
  would be invented. What is reported is whether the test was ordered at all,
  crossed with whether medication changed, against the readmission outcome.
- **No time trends.** There are no admission dates. `encounter_id` orders rows
  but carries no calendar meaning, so a chart of "readmissions over time" would
  be plotting row order.
- **Nothing here is causal.** Medication is changed for patients who are doing
  worse, so the higher readmission rate in that group reflects who was selected
  into it. The gap is real and statistically separable on 69,987 patients
  (p = 0.000127) and it is still under one percentage point, and it is still not
  an effect of the treatment.

One more limit that is about the platform rather than the data: every model
number here is measured offline on all 51 features, while the API supplies 7.
The offline figures are the model's, not the API's, and are labelled that way
...
**Evidence charts (`ml/src/evaluation/make_m3_charts.py`, new).** Draws four PNGs
from the artefacts into `docs/06-milestones/evidence/`. It reads the JSON rather
than recomputing anything, so a chart cannot drift from the numbers the backend
is served.
...
../backend/.venv/bin/python -m src.evaluation.make_m3_charts     # writes the evidence PNGs
...
Generated by `ml/src/evaluation/make_m3_charts.py` directly from the two
artefacts, so a chart can never disagree with the JSON the backend reads. Each
file is under 100 KB.

![Readmission by whether medication was changed](evidence/m3-readmission-by-medication-change.png)

![Readmission by whether HbA1c was tested](evidence/m3-readmission-by-a1c-tested.png)

![Top 10 SHAP risk drivers](evidence/m3-top-shap-drivers.png)

![Recovery score by primary diagnosis](evidence/m3-recovery-by-diagnosis-cohort.png)

The first two are dot-and-interval plots rather than bar charts on purpose: the
confidence interval is the point, and a bar would draw the eye to a length
measured from zero while hiding the uncertainty that decides whether the gap
means anything. In the diagnosis chart the `missing` cohort is drawn grey and
carries its own note - `n=10, too few to rely on` - so the warning is in words on
the chart and does not depend on the reader knowing what the colour means.

### Full run log

`evidence/m3-run.txt` is the captured output of the whole milestone: the pipeline
building both artefacts, the charts rendering, all 142 tests, ruff and black. It
records the interpreter and the scikit-learn, xgboost, shap and pandas versions
at the top, because the saved model only loads under scikit-learn 1.6.0.
...
Writing charts to docs/06-milestones/evidence/
  m3-readmission-by-medication-change.png  50 KB
  m3-readmission-by-a1c-tested.png  48 KB
  m3-top-shap-drivers.png  81 KB
  m3-recovery-by-diagnosis-cohort.png  60 KB
...
test_metrics 17, test_explain 13, test_preprocess 11, test_charts 11,
test_train 9, test_serving_skew 7, test_n3_feature_levers 6, test_config 4. The
suite was 49 tests before this milestone, so 93 are new.

The full transcript, including the per-test names, is in `evidence/m3-run.txt`.
